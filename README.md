...
一个面向程序员日常使用的桌面格式化工具，用来快速处理 `JSON` / `XML` 的格式化、压缩、复制、导入导出与专注查看。

项目基于 `Tauri + React + TypeScript + Tailwind CSS + Monaco Editor` 构建，目标是提供一个真正顺手、打开即用、离线可用的桌面应用，而不是临时的网页 Demo。
...
- `src/components`
  负责页面结构、工具栏、编辑区、状态栏、历史记录和输出区专注查看弹层
- `src/hooks`
  封装剪贴板、文件传输、主题、窗口控制、格式化状态等逻辑
- `src/utils`
  负责 JSON / XML 处理、错误信息整理、文本统计、Monaco 编辑器配置等工具函数
- `src-tauri`
...
当前版本重点覆盖程序员高频使用的桌面格式化场景：

- 默认带示例内容，打开即可操作
- 输入区与输出区都可直接编辑
- 输出区支持专注查看，便于阅读长内容
- 错误会以可读方式显示，而不是直接抛技术堆栈
- 桌面版中的文件导入 / 导出、剪贴板访问、窗口置顶都走原生能力
